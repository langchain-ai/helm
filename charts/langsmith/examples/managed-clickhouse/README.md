...
This folder can be used to setup a replicated clickhouse setup on a kubernetes cluster.

### Steps
1. Install the clickhouse operator. Run the command below after replacing `<langsmith-namespace>` with the namespace you will be deploying the replicated clickhouse cluster into.
```
...
```
4. After the replicated clickhouse cluster is setup, you can then configure the appropriate LangSmith values. Here is an example if you do not change any of the fields in replicated-clickhouse-3-node-config.yaml, but fill in `<langsmith-namespace>`:
```
clickhouse:
  external:
    enabled: true

...
| config.sandboxes.sandboxHost.daemonSet.nodeSelector | object | `{}` | Node selector for sandbox-host and sandbox-host-installer pods. Empty uses the default selector `sandbox.langsmith.com/host=true`; set this map to replace the default if your sandbox node pool uses different labels. |
| config.sandboxes.sandboxHost.daemonSet.tolerations | list | `[{"effect":"NoSchedule","key":"sandbox.langsmith.com/host","operator":"Equal","value":"true"}]` | Tolerations for sandbox-host and sandbox-host-installer pods. The default expects sandbox-host nodes tainted `sandbox.langsmith.com/host=true:NoSchedule`; override this if your sandbox node pool uses different taints. |
...
| config.sandboxes.xServiceAuthJwtSecretPrevious | string | `""` | Optional previous service-auth secret used during rotation. Used only when this chart creates the LangSmith app Secret. When config.existingSecretName is set, add key `sandbox_x_service_auth_jwt_secret_previous` to that Secret instead. |

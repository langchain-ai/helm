...
| config.sandboxes.smithboxControl.redis.connectionUrlSecretKey | string | `""` | Key in the sandbox Redis Secret containing the Redis connection URL. Empty uses `redis.external.connectionUrlSecretKey`. |
| config.sandboxes.smithboxControl.redis.existingSecretName | string | `""` | Existing Secret in `config.sandboxes.namespace` containing the non-cluster Redis connection URL for smithbox-control. Required when non-cluster Redis credentials are managed outside this chart with `redis.external.existingSecretName` or `redis.disableSecretCreation`. |

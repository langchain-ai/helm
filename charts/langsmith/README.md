...
| config.security | object | `{"cors":{"allowedOrigins":"*","allowedOriginsRegex":"","alwaysAllowPathsRegex":""},"ssrf":{"allowPrivateIpsMcpServers":false,"allowPrivateIpsTools":false,"allowPrivateIpsWebhooks":false}}` | Security configuration for CORS, headers, and other security-related settings. These settings control cross-origin access and help protect against common web vulnerabilities. |
...
| config.security.ssrf | object | `{"allowPrivateIpsMcpServers":false,"allowPrivateIpsTools":false,"allowPrivateIpsWebhooks":false}` | SSRF (Server-Side Request Forgery) protection configuration. Controls whether private IP ranges (RFC 1918) are allowed for specific surfaces. Cloud metadata, localhost, and Kubernetes internal DNS protections remain enforced regardless of these settings. |
| config.security.ssrf.allowPrivateIpsMcpServers | bool | `false` | Allow MCP servers to use private IP addresses for CRUD, proxy, and agent builder MCP client operations. Enable this if your self-hosted deployment connects to MCP servers on internal networks. |
| config.security.ssrf.allowPrivateIpsTools | bool | `false` | Allow tools.json mcp_server_url validation to accept private IP addresses. Enable this if your self-hosted deployment references MCP server URLs on internal networks. |
| config.security.ssrf.allowPrivateIpsWebhooks | bool | `false` | Allow webhooks (run rules, prompt, alert, agent builder) to target private IP addresses. Enable this if your self-hosted deployment needs to deliver webhooks to internal services. |

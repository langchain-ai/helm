...
| sandboxes.sandboxHost.deployment.nodeSelector | object | `{"sandbox.langsmith.com/host":"true"}` | Node selector for sandbox-host pods, matching the labels on your sandbox node pool. Helm merges maps, so replacing the default means setting `sandbox.langsmith.com/host: null` alongside your own key. |

...
| config.sandboxes.juicefs.csi.configSecretName | string | `"juicefs-csi-config"` | Name of the chart-managed JuiceFS CSI config Secret. The bundled JuiceFS CSI setup is intended only for JuiceFS PVs in the namespace where this chart is installed. Updating this Secret does not refresh existing mounts automatically; restart workloads that mount the JuiceFS PVC, or delete the affected JuiceFS mount pods so they are recreated with the updated Secret. |

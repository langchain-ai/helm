...
| sandboxes.juicefs.hostMount.cacheSizeLimit | string | `""` | Maximum node-local emptyDir space available to the JuiceFS cache. Empty leaves the emptyDir uncapped; JuiceFS still enforces cache-size from mountOptions. |
...
| sandboxes.juicefs.hostMount.mountOptions | list | `["--cache-dir=/var/cache/juicefs","--cache-size=51200","--cache-large-write"]` | Complete JuiceFS CLI options passed to sandbox-host. The defaults use a conservative 50 GiB node-local cache; tune this and cacheSizeLimit together for the sandbox node pool. |

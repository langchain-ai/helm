...
| sandboxes.sandboxHost.deployment.nodeSelector | object | `{}` | Node selector for sandbox-host pods. Required when sandboxes are enabled: host pods must land on KVM-capable nodes, and a toleration alone permits that without attracting them. Label your sandbox pool `sandbox.langsmith.com/host=true` and set that here, or match whatever labels your pool already carries. |
| sandboxes.sandboxHost.deployment.readinessProbe | object | `{"failureThreshold":3,"initialDelaySeconds":5,"periodSeconds":10,"tcpSocket":{"port":"http"},"timeoutSeconds":3}` | Readiness probe. The host binds its listener only once its engine, fence and lease loops are up, so accepting connections is the signal a rollout should wait on. There is deliberately no liveness probe: a restart mid-suspend would destroy running microVMs. |
...
| sandboxes.sandboxHost.deployment.securityContext | object | `{"privileged":true}` | Container security context. sandbox-host must stay privileged: it creates network namespaces, programs iptables, and propagates mounts for the JuiceFS volume. A read-only root filesystem will not work either, since it unpacks the guest tools image on startup. |

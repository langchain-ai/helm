...
| config.langchainEnv | string | `"local_kubernetes"` | Runtime environment value rendered as LANGCHAIN_ENV for LangSmith services and sandbox runtime components. |
...
| config.sandboxes | object | `{"callbackSigningJwk":"","clusterName":"","defaultBlueprintImage":"","enabled":false,"firecrackerSizeClasses":[{"memoryMib":1792,"name":"small","vcpus":1},{"memoryMib":3840,"name":"medium","vcpus":1},{"memoryMib":7936,"name":"large","vcpus":2},{"memoryMib":16128,"name":"xlarge","vcpus":4}],"juicefs":{"bucket":"","csi":{"configSecretName":"juicefs-csi-config","existingSecretName":"","mountPath":"/juicefs","pvName":"smithbox-juicefs-csi","pvcName":"smithbox-juicefs-csi"},"name":"sandbox-juicefs","redis":{"metaURL":""},"storage":"s3"},"limits":{"maxCpuCores":16,"maxEphemeralStorageGb":100,"maxMemoryGb":64,"maxSandboxes":1000,"minEphemeralStorageGb":1},"proxyCa":{"certManager":{"createIssuer":true,"duration":"8760h","issuerKind":"Issuer","issuerName":"smithbox-proxy-selfsigned","renewBefore":"720h"},"existingSecretName":"","mode":"generatedSecret","secretName":"smithbox-proxy-ca"},"sandboxHost":{"containerPort":19190,"name":"sandbox-host","serviceAccount":{"annotations":{},"automountServiceAccountToken":true,"create":true,"labels":{},"name":""},"statefulSet":{"annotations":{},"labels":{},"nodeSelector":{},"podAnnotations":{},"replicas":null,"resources":{"requests":{"cpu":"2","memory":"2Gi"}},"securityContext":{"privileged":true},"terminationGracePeriodSeconds":300,"tolerations":[{"effect":"NoSchedule","key":"sandbox.langsmith.com/host","operator":"Equal","value":"true"}]}},"sandboxHostInstaller":{"daemonSet":{"annotations":{},"labels":{},"maxUnavailable":1,"podAnnotations":{},"resources":{"limits":{"cpu":"200m","memory":"128Mi"},"requests":{"cpu":"10m","memory":"32Mi"}},"securityContext":{"runAsUser":0},"timeout":"120s"},"name":"sandbox-host-installer"},"serviceUrlBaseUrl":"","smithboxControl":{"containerPort":9090,"deployment":{"annotations":{},"labels":{},"nodeSelector":{},"podAnnotations":{},"replicas":2,"resources":{"limits":{"cpu":"2","memory":"8Gi"},"requests":{"cpu":"2","memory":"8Gi"}},"securityContext":{"privileged":true},"tolerations":[]},"hostObserverEnabled":true,"name":"smithbox-control","redisDisableClientSetinfo":false,"runtimeLifecycleReconcilerEnabled":false,"service":{"annotations":{},"labels":{},"port":9090,"type":"ClusterIP"},"serviceAccount":{"annotations":{},"automountServiceAccountToken":true,"create":true,"labels":{},"name":""}},"xServiceAuthJwtSecret":"","xServiceAuthJwtSecretPrevious":""}` | LangSmith Sandboxes configuration. Disabled by default. When enabled, this installs sandbox runtime resources in the same namespace as the LangSmith release and wires the LangSmith API/frontend services to expose sandbox functionality. This supports the same-cluster sandbox-host architecture only. Enabling sandboxes also installs the JuiceFS CSI driver. The CSI driver creates cluster-scoped resources, so only one sandbox-enabled LangSmith release should manage it per Kubernetes cluster. Do not enable sandboxes in multiple releases in the same cluster, or alongside an independently managed JuiceFS CSI installation, unless you have verified ownership of those cluster-scoped resources. |
| config.sandboxes.callbackSigningJwk | string | `""` | Optional private JWK for sandbox callback signing. Used only when this chart creates the LangSmith app Secret. When config.existingSecretName is set, add optional key `sandbox_callback_signing_jwk` to that Secret to enable callback signing. |
...
| config.sandboxes.firecrackerSizeClasses | list | `[{"memoryMib":1792,"name":"small","vcpus":1},{"memoryMib":3840,"name":"medium","vcpus":1},{"memoryMib":7936,"name":"large","vcpus":2},{"memoryMib":16128,"name":"xlarge","vcpus":4}]` | Required when sandboxes are enabled. Firecracker VM size classes passed to smithbox-control as `SMITHBOX_FIRECRACKER_SIZE_CLASSES`; each entry must include `name`, `vcpus`, and `memoryMib`. |
...
| config.sandboxes.proxyCa.mode | string | `"generatedSecret"` | generatedSecret creates a self-signed CA Secret with Helm and reuses it on live upgrades via lookup. In pure render/GitOps workflows where lookup cannot read the cluster, generatedSecret produces different cert material on each render; use certManager or existingSecret for deterministic manifests. Other modes are existingSecret or certManager. |
| config.sandboxes.sandboxHost.statefulSet.nodeSelector | object | `{}` | Node selector for sandbox-host and sandbox-host-installer pods. Empty uses the default selector `sandbox.langsmith.com/host=true`; set this map to replace the default if your sandbox node pool uses different labels. |
| config.sandboxes.sandboxHost.statefulSet.replicas | string | `nil` | Optional initial sandbox-host StatefulSet replica count. Leave null to let Kubernetes default the initial replica count and avoid Helm reconciling runtime scaling. |
| config.sandboxes.sandboxHost.statefulSet.tolerations | list | `[{"effect":"NoSchedule","key":"sandbox.langsmith.com/host","operator":"Equal","value":"true"}]` | Tolerations for sandbox-host and sandbox-host-installer pods. The default expects sandbox-host nodes tainted `sandbox.langsmith.com/host=true:NoSchedule`; override this if your sandbox node pool uses different taints. |
...
| config.sandboxes.xServiceAuthJwtSecret | string | `""` | Shared service-auth secret for LangSmith <-> sandbox runtime calls. Used only when this chart creates the LangSmith app Secret. When config.existingSecretName is set, add key `sandbox_x_service_auth_jwt_secret` to that Secret instead. |

...
| config.sandboxes.sandboxHost.autoscaling | object | `{"enabled":false,"headroomHosts":1,"maxReplicas":10,"minReplicas":1,"scaleDownStabilizationSeconds":300,"targetUtilizationPercent":70}` | Sandbox host pool autoscaling. There is no HPA or KEDA object here: the elected sandbox-host sizes the sandbox-host Deployment itself from observed pool load, because only it knows how much CPU is committed to live microVMs. The chart renders this config as Deployment annotations rather than pod env so retuning is an in-place patch that does not roll the hosts and tear down running sandboxes. Set `sandbox-host.smith.langchain.com/autoscale-override` on the Deployment manually to freeze (`off`) or pin (an integer) the pool as a break-glass; the chart never manages that key. |

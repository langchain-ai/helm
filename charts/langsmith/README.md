...
| config.sandboxes.callbackSigningJwk | string | `""` | Required private JWK for sandbox callback signing when this chart creates the LangSmith app Secret. When config.existingSecretName is set, add key `sandbox_callback_signing_jwk` to that Secret instead. Signed egress callbacks also need config.hostname for the public issuer; without it, signing fails closed. |
...
| config.sandboxes.juicefs.csi.install | bool | `true` | Install the bundled JuiceFS CSI driver. Set to false when the cluster already runs one: the chart then renders only the sandbox PV/PVCs and CSI config Secret and binds them to the existing driver. Leaving it true alongside an independent installation fails the install on the shared upstream names. |
...
| config.sandboxes.sandboxHost.autoscaling | object | `{"enabled":false,"headroomHosts":1,"maxReplicas":10,"minReplicas":1,"scaleDownStabilizationSeconds":300,"targetUtilizationPercent":70}` | Sandbox host pool autoscaling. There is no HPA or KEDA object: the elected sandbox-host sizes the Deployment itself, since only it knows how much CPU is committed to live microVMs. These are the baseline; `sandbox-host.smith.langchain.com/autoscale-*` annotations on the Deployment (including `autoscale-override` to freeze or pin the pool) override them live and are never managed by the chart. |
...
| config.sandboxes.sandboxHost.deployment.replicas | int | `1` | Number of sandbox-host replicas, i.e. how many sandbox nodes back the pool. Ignored when autoscaling is enabled. Hosts are one per node (required anti-affinity on `kubernetes.io/hostname` plus `hostPort`), so this must not exceed the sandbox node count. Image updates roll one node at a time (`maxSurge: 0`), since a surge pod could never schedule. |
...
| config.sandboxes.sandboxHost.pdb | object | `{"annotations":{},"enabled":false,"labels":{},"maxUnavailable":1}` | Disruption budget for sandbox-host. Each drain suspends every microVM on that host, so cap concurrent evictions rather than letting a multi-node drain empty the pool at once. maxUnavailable rather than minAvailable, since the pool size moves under the autoscaler and a small pool must stay drainable. Setting minAvailable overrides it. |

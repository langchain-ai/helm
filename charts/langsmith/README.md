...
| config.sandboxes.juicefs.csi.configSecretName | string | `"juicefs-csi-config"` | Name of the chart-managed JuiceFS CSI config Secret. The bundled JuiceFS CSI setup is intended only for JuiceFS PVs in the namespace where this chart is installed. Chart-managed Secret/config changes roll sandbox-host through checksum annotations, but externally managed Secret content and existing JuiceFS mount pods are not refreshed automatically; restart workloads that mount the JuiceFS PVC, or delete the affected JuiceFS mount pods so they are recreated with the updated Secret. |

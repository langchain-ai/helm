...
| config.sandboxes.sandboxHost.name | string | `"sandbox-host"` | Name of the sandbox-host Deployment. Unlike other components this is not prefixed with the release fullname, because sandbox-host resolves its own Deployment by this fixed name. Do not change it. |

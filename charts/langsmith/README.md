...
| config.sandboxes.juicefs.csi.configSecretName | string | `"juicefs-csi-config"` | Name of the chart-managed JuiceFS CSI config Secret. The chart-managed CSI deployment intentionally uses a static JuiceFS CE `metaurl` Secret with JuiceFS client config caching disabled, which keeps Secret and pod exec permissions scoped to the LangSmith release namespace. If your deployment requires JuiceFS Enterprise/token-based initconfig refresh or dynamic JuiceFS PVs outside this release namespace, manage JuiceFS CSI outside this sandbox feature. |

...
| config.sandboxes | object | `{"callbackSigningJwk":"","clusterName":"","enabled":false,"juicefs":{"bucket":"","csi":{"configSecretName":"juicefs-csi-config","controller":{"annotations":{},"serviceAccount":{"annotations":{}}},"existingSecretName":"","install":true,"mountPath":"/juicefs","mountPodPatch":[{"mountOptions":["cache-dir=/var/cache/juicefs-csi","cache-size=51200","buffer-size=300","prefetch=3","metrics=0.0.0.0:9567"],"resources":{"limits":{"cpu":"2","memory":"4Gi"},"requests":{"cpu":"2","memory":"4Gi"}}},{"mountOptions":["cache-dir=/var/cache/juicefs-csi","cache-size=307200"],"pvcSelector":{"matchLabels":{"juicefs.langsmith.com/cache":"ssd"}}}],"node":{"annotations":{},"serviceAccount":{"annotations":{}}},"pvName":"smithbox-juicefs-csi","pvcName":"smithbox-juicefs-csi"},"name":"sandbox-juicefs","redis":{"metaURL":""},"storage":"s3"},"limits":{"maxCpuCores":16,"maxEphemeralStorageGib":100,"maxMemoryGb":64,"maxSandboxes":1000,"minEphemeralStorageGb":1},"proxyCa":{"existingSecretName":"","mode":"generatedSecret","secretName":"smithbox-proxy-ca"},"sandboxHost":{"allowPrivateIPCallbacks":false,"autoscaling":{"enabled":false,"headroomHosts":1,"maxReplicas":10,"minReplicas":1,"scaleDownStabilizationSeconds":300,"targetUtilizationPercent":70},"containerPort":19190,"deployment":{"annotations":{},"extraEnv":[],"labels":{},"nodeSelector":{},"podAnnotations":{},"replicas":1,"resources":{"requests":{"cpu":"2","memory":"2Gi"}},"securityContext":{"privileged":true},"terminationGracePeriodSeconds":300,"tolerations":[{"effect":"NoSchedule","key":"sandbox.langsmith.com/host","operator":"Equal","value":"true"}]},"name":"sandbox-host","pdb":{"annotations":{},"enabled":false,"labels":{},"maxUnavailable":1},"rbac":{"annotations":{},"create":true,"labels":{}},"serviceAccount":{"annotations":{},"automountServiceAccountToken":true,"create":true,"labels":{},"name":""}},"serviceUrlBaseUrl":""}` | LangSmith Sandboxes configuration. Disabled by default. When enabled, this installs sandbox runtime resources in the same namespace as the LangSmith release and wires the LangSmith API/frontend services to expose sandbox functionality. This supports the same-cluster sandbox-host architecture only. Self-hosted sandboxes are currently supported on AWS/EKS and GCP/GKE. Azure/AKS is supported by the base LangSmith chart, but not by sandboxes yet. Enabling sandboxes also installs the JuiceFS CSI driver. The CSI driver creates cluster-scoped resources, so only one sandbox-enabled LangSmith release should manage it per Kubernetes cluster. Do not enable sandboxes in multiple releases in the same cluster, or alongside an independently managed JuiceFS CSI installation, unless you have verified ownership of those cluster-scoped resources. |
| config.sandboxes.callbackSigningJwk | string | `""` | Required private JWK for sandbox callback signing when this chart creates the LangSmith app Secret. When config.existingSecretName is set, add key `sandbox_callback_signing_jwk` to that Secret instead. Signed egress callbacks also need config.hostname: it supplies the public issuer receivers use to fetch the verification key from `<issuer>/.well-known/jwks.json`. Without it, callback signing fails closed. |
...
| config.sandboxes.juicefs.csi.install | bool | `true` | Install the bundled JuiceFS CSI driver. Set to false if this cluster already runs the JuiceFS CSI driver (for example via the upstream juicefs-csi-driver chart): the chart then renders only the sandbox PV/PVCs and the CSI config Secret and binds them to the existing driver. Leaving this true alongside an independent installation fails the install, because the bundled copy uses the same cluster-scoped CSIDriver and ClusterRole names as upstream. |
...
| config.sandboxes.sandboxHost.autoscaling | object | `{"enabled":false,"headroomHosts":1,"maxReplicas":10,"minReplicas":1,"scaleDownStabilizationSeconds":300,"targetUtilizationPercent":70}` | Sandbox host pool autoscaling. There is no HPA or KEDA object here: the elected sandbox-host sizes the sandbox-host Deployment itself from observed pool load, because only it knows how much CPU is committed to live microVMs. These values are the baseline the host boots with. The same knobs, plus `sandbox-host.smith.langchain.com/autoscale-override` to freeze (`off`) or pin (an integer) the pool, can be set as `sandbox-host.smith.langchain.com/autoscale-*` annotations on the Deployment for a live change that does not roll the hosts; the chart never manages those. |
| config.sandboxes.sandboxHost.autoscaling.headroomHosts | int | `1` | Spare empty hosts kept above current demand so a sandbox create rarely waits for a cold node. |
| config.sandboxes.sandboxHost.autoscaling.scaleDownStabilizationSeconds | int | `300` | Hold the replica target at its recent peak for this long before shrinking, so load dips do not thrash the pool. |
| config.sandboxes.sandboxHost.autoscaling.targetUtilizationPercent | int | `70` | Target share of pool CPU capacity committed to sandboxes before scaling up. |
...
| config.sandboxes.sandboxHost.deployment.replicas | int | `1` | Number of sandbox-host replicas, i.e. how many sandbox nodes back the pool. Ignored when config.sandboxes.sandboxHost.autoscaling.enabled is true, so Helm does not fight the autoscaler on upgrades. Replicas are kept one per node by chart-managed required pod anti-affinity on `kubernetes.io/hostname` plus the sandbox-host `hostPort`, so this must not exceed the number of sandbox nodes. The chart also uses a no-surge rolling update strategy (`maxSurge: 0`, `maxUnavailable: 1`) so image updates roll hosts one node at a time instead of creating unschedulable surge pods. |
...
| config.sandboxes.sandboxHost.name | string | `"sandbox-host"` | Component name for sandbox-host. The Deployment, PDB and RBAC are named `<release-fullname>-<name>`, so multiple releases can coexist. The chart passes the resulting name to the host as SANDBOX_HOST_DEPLOYMENT_NAME, which requires a sandbox-host image that reads it. |
| config.sandboxes.sandboxHost.pdb | object | `{"annotations":{},"enabled":false,"labels":{},"maxUnavailable":1}` | Disruption budget for sandbox-host. Hosts are one-per-node and each drain suspends every microVM on that host, so cap concurrent voluntary evictions rather than letting a multi-node drain empty the pool at once. maxUnavailable is used instead of minAvailable because the pool size changes under the autoscaler. |

...
| connector.podSecurityContext | object | `{"runAsNonRoot":true,"seccompProfile":{"type":"RuntimeDefault"}}` | Pod-level security context, merged over `commonPodSecurityContext`. |
...
| connector.targets | object | `{}` | Target name to address. HTTP targets carry a scheme (`https://svc.internal:8443`); TCP targets are `host:port`. Each entry becomes part of LANGSMITH_CONNECTOR_TARGETS. |
